...
class MarkdownManager:
    ...
    def render(self, rel_path: str) -> dict:
        ...
        return {"html": html, "toc": toc, "title": title, "rel_path": rel_path}
...
def _build_tree(directory: pathlib.Path, root: pathlib.Path) -> list[FileNode]:
    ...
    for entry in entries:
        ...
        if entry.is_dir():
            ...
            if children:
                nodes.append(FileNode(name=entry.name, path=rel, abs_path=str(entry), is_dir=True, children=children))
        elif entry.suffix.lower() in {".md", ".markdown"}:
            nodes.append(FileNode(name=entry.name, path=rel, abs_path=str(entry), is_dir=False))
    return nodes
